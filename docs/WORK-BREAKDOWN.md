...
| F4 | **CD on GitHub Actions — two artifacts on two events.** A pull request builds a **dev** artifact so a reviewer can open the thing rather than read about it; a merge to `main` builds a **release** artifact from the version already bumped on the branch (the cadence in `docs/PLAN.md` §9 puts that bump as the last commit before the merge, which is exactly what a release job needs) | strictly after F1–F3; a release pipeline without a signed artifact publishes something nobody can open |
...
| F10 | **Whisper inside the app** — the `whisper-cli` binary, its libraries and `ggml-small.bin` shipped with the build, so the install is one step instead of four | the delivery is a dev build, and vendoring a binary is a packaging decision that belongs after F1. Sizes measured, not estimated: `whisper-cli` is 643 KB and its libraries about 1.1 MB, read off the development machine; the model sizes come from the publisher's `content-length`, and the `base.en` figure matches the file on this disk byte for byte. `base.en` 141 MB (today) · `small.en` 465 MB · `small` 465 MB (requested). So a ~2 MB build becomes roughly **470 MB**, accepted on purpose. Two things to decide deliberately inside it: `small` and `small.en` are the same size and `small` is the MULTILINGUAL build, while everything else here is English-only — the transcriber is invoked `-l en`, the voice is chosen English on purpose, and `i18n/` holds one file, so multilingual `small` behind `-l en` buys only bytes (making it real is F7). And whatever is vendored must stay `ffmpeg`-free: GPL-3.0-or-later reaches further into a shipped product than anything else here, and that is a property of the binary, not of this repository, so it needs checking at the moment of vendoring |
...
| F16 | **Sign in as any account**, instead of inheriting whichever account Claude Code is signed into on this machine | the app authenticates by *not* authenticating: it checks that Claude Code's own Keychain entry exists and never reads it (`README.md` → Requirements). That is the cheapest correct thing for a demonstration build and the wrong thing for a product, for two separate reasons — the user cannot choose the account, and **Claude Code has to be installed at all**, because the app has no path of its own to a model |
| F20 | **Clearing the agent window clears the context** — emptying the panel ends the session rather than only stopping it being drawn | today they are separate things: `sessionId` is carried in `useTurn` across turns, on purpose, so that *"add milk"* and *"what's on my list?"* are one conversation across two `claude -p` spawns. Nothing in the interface drops it, so a user who clears the panel believes they started over and the agent has not — which is worse than having no clear at all, because it is a false statement made by the UI |
...
| F18 | **The consent is not recorded against this application at all** — System Settings shows Electron's name and icon, and every Electron app on the machine shares that one switch | an unpackaged development build has no bundle identifier of its own, so it *is* Electron as far as macOS is concerned. Three consequences, none cosmetic: the user cannot tell what they are granting, the switch is shared, and a grant given to VoiceDesk is really given to anything else built on Electron. A bundle identifier is created at packaging time, so this is fixed by F1 and F2 and by nothing short of them |
